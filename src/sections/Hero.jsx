...
const highlights = [
  { label: "DevOps Intern", description: "Observability & Azure at CNH ITC" },
  { label: "Dual Degree", description: "Physics & Mechanical Engineering at BITS Pilani" },
  { label: "Focus", description: contact.focus },
];
...
export function Hero() {
  return (
    ...
    >
      {/* background blobs */}
      <div className="absolute -left-32 top-16 h-80 w-80 rounded-full bg-ashish-200/40 blur-3xl" aria-hidden />
      <div className="absolute -right-10 top-0 h-72 w-72 rounded-full bg-teal-200/40 blur-3xl" aria-hidden />
      <div className="absolute bottom-0 left-1/3 h-64 w-64 rounded-full bg-amber-200/30 blur-3xl" aria-hidden />
          ...
          <div className="grid gap-4 rounded-3xl border border-slate-200/80 bg-white/70 p-6 shadow-sm shadow-slate-100 backdrop-blur md:grid-cols-3">
            {highlights.map((highlight) => (
              <div key={highlight.label} className="space-y-1">
                <p className="text-xs font-semibold uppercase tracking-wide text-ashish-500/80">
                  {highlight.label}
                </p>
